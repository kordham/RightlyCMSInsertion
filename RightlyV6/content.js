...
chrome.storage.local.get(['STATE'], function(result) {
    if (result.STATE == 1) {
		...
		if (response.cur_url == "https://www.rightly.co.uk/cms/companies/company/") {
			...
			document.getElementsByClassName('right')[0].innerHTML+='<input type="file" id="file">';
...
function csvToArray(data, delimeter) {
    ...
    return fivefinder(array);
}


function fivefinder(array){
    var fiver = new Array();
...
function populate() {
  ...
  console.log(reader.readAsText(file));
	...
	x = "'"+e.target.result.replace(/(\r\n|\n|\r)/gm,'\n')+"'";
  	//console.log(x);




    while(x.endsWith('n') || x.endsWith(',,')){
        m = m.pop().pop()
      }
